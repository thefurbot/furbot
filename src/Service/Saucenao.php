...
class Saucenao
{
    ...
    {
        $this->apikey = $params->get('saucenao.apikey');
        $this->acceptThreshold = $params->get('saucenao.accept');
    }
    ...
    {
        $url = 'https://saucenao.com/search.php?output_type=2&api_key=' . $this->apikey . '&url=' . urlencode($url);
        ...
        foreach ($results as $result) {
            if (!empty($result['data']['ext_urls'])) foreach ($result['data']['ext_urls'] as $u) {
                if ($result['header']['similarity'] < $this->acceptThreshold) {
                    continue;
                }
